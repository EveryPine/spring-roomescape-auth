...
| `409` | 이미 존재하는 이용자입니다. | 같은 아이디를 가진 이용자가 이미 존재하는 경우 |
...
```json
{
  "token": "eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJtZW1iZXJJZDEyMyIsInJvbGUiOiJVU0VSIn0.signature"
}
```
...
|:-----:|:----------------------|:--------------------------|
| `400` | 요청 형식이 잘못되었습니다.       | 데이터 형식이 틀리거나 필수 값이 누락된 경우 |
| `401` | 아이디나 비밀번호가 일치하지 않습니다. | 아이디 또는 비밀번호가 일치하지 않는 경우   |
...
|:-----:|:----------------------------------------------|:--------------------------|
| `400` | 요청 형식이 잘못되었습니다.                               | 데이터 형식이 틀리거나 필수 값이 누락된 경우 |
| `401` | 인증되지 않은 이용자입니다. / 만료된 토큰입니다. / 유효하지 않은 토큰입니다. | 토큰이 없거나 만료되었거나 유효하지 않은 경우 |
...
| `401` | 인증되지 않은 이용자입니다. / 만료된 토큰입니다. / 유효하지 않은 토큰입니다. | 토큰이 없거나 만료되었거나 유효하지 않은 경우                       |
| `403` | 접근할 권한이 없습니다.                                 | 관리자 권한이 없는 경우                                   |
| `409` | 이미 존재하는 예약입니다.                                | 같은 지점, 날짜, 시간 및 테마를 가진 예약이 존재하는 경우              |
...
| `400` | 요청 형식이 잘못되었습니다.                               | 데이터 형식이 틀리거나 필수 파라미터가 누락된 경우, 참조 id가 존재하지 않는 경우 |
| `401` | 인증되지 않은 이용자입니다. / 만료된 토큰입니다. / 유효하지 않은 토큰입니다. | 토큰이 없거나 만료되었거나 유효하지 않은 경우                       |
| `403` | 요청한 예약에 접근할 권한이 없습니다.                         | 매니저 권한이 없는 경우, 다른 매장의 예약 생성을 시도하는 경우            |
| `409` | 이미 존재하는 예약입니다.                                | 같은 지점, 날짜, 시간 및 테마를 가진 예약이 존재하는 경우              |
| `422` | 지난 예약은 접근할 수 없습니다.                            | 지난 날짜 및 시간에 예약을 시도하는 경우                         |
...
|:-----:|:----------------------------------------------|:--------------------------|
| `401` | 인증되지 않은 이용자입니다. / 만료된 토큰입니다. / 유효하지 않은 토큰입니다. | 토큰이 없거나 만료되었거나 유효하지 않은 경우 |
| `403` | 요청한 예약에 접근할 권한이 없습니다.                         | 매니저 권한이 없는 경우             |
...
| `409` | 이미 존재하는 예약입니다.                                | 같은 지점에서 변경하려는 날짜 또는 시간에 이미 예약이 존재하는 경우          |
...
|:-----:|:---------------------------|:-----------------------------|  
| `400` | 요청 형식이 잘못되었습니다.            | 데이터 형식이 틀리거나 필수 파라미터가 누락된 경우 |
| `400` | 조회 개수는 1 이상이어야 합니다.        | limit이 음수거나 0인 경우            |
