package roomescape.global.error;

import org.springframework.http.HttpStatus;

public enum ErrorCode {

    COMMON_INVALID_REQUEST(HttpStatus.BAD_REQUEST, "요청 형식이 잘못되었습니다."),
    COMMON_INVALID_PARAMETER_TYPE(HttpStatus.BAD_REQUEST, "요청 파라미터의 형식이 잘못되었습니다."),
    COMMON_INVALID_REQUEST_BODY(HttpStatus.BAD_REQUEST, "요청 본문의 형식이 잘못되었습니다."),

    AUTH_LOGIN_FAILED(HttpStatus.UNAUTHORIZED, "아이디나 비밀번호가 일치하지 않습니다."),
    AUTH_EXPIRED_TOKEN(HttpStatus.UNAUTHORIZED, "만료된 토큰입니다."),
    AUTH_INVALID_TOKEN(HttpStatus.UNAUTHORIZED, "유효하지 않은 토큰입니다."),
    AUTH_UNAUTHORIZED(HttpStatus.UNAUTHORIZED, "인증되지 않은 이용자입니다."),
    AUTH_FORBIDDEN(HttpStatus.FORBIDDEN, "접근할 권한이 없습니다."),

    MEMBER_NOT_FOUND(HttpStatus.NOT_FOUND, "요청한 이용자가 존재하지 않습니다."),
    MEMBER_DUPLICATE(HttpStatus.CONFLICT, "이미 존재하는 이용자입니다."),

    STORE_NOT_FOUND(HttpStatus.NOT_FOUND, "요청한 지점을 찾을 수 없습니다."),

    RESERVATION_FORBIDDEN(HttpStatus.FORBIDDEN, "요청한 예약에 접근할 권한이 없습니다."),
    RESERVATION_NOT_FOUND(HttpStatus.NOT_FOUND, "요청한 예약을 찾을 수 없습니다."),
    RESERVATION_DUPLICATE(HttpStatus.CONFLICT, "이미 존재하는 예약입니다."),
    RESERVATION_ALREADY_PASSED(HttpStatus.UNPROCESSABLE_ENTITY, "지난 예약은 접근할 수 없습니다."),
    RESERVATION_TIME_ALREADY_PASSED(HttpStatus.UNPROCESSABLE_ENTITY, "지난 시점으로는 변경할 수 없습니다."),

    TIME_NOT_FOUND(HttpStatus.NOT_FOUND, "요청한 시간을 찾을 수 없습니다."),
    TIME_DUPLICATE(HttpStatus.CONFLICT, "이미 존재하는 시간입니다."),
    TIME_REFERENCED_BY_RESERVATION(HttpStatus.CONFLICT, "요청한 시간을 참조하는 예약이 존재합니다."),
    TIME_INVALID_DATE(HttpStatus.UNPROCESSABLE_ENTITY, "지난 날짜로 시간 조회를 시도했습니다."),

    THEME_NOT_FOUND(HttpStatus.NOT_FOUND, "요청한 테마를 찾을 수 없습니다."),
    THEME_DUPLICATE(HttpStatus.CONFLICT, "이미 존재하는 테마입니다."),
    THEME_REFERENCED_BY_RESERVATION(HttpStatus.CONFLICT, "요청한 테마를 참조하는 예약이 존재합니다."),
    THEME_INVALID_DATE(HttpStatus.UNPROCESSABLE_ENTITY, "시작 일자는 종료 일자 이전 시점이어야 합니다.");

    private final HttpStatus status;
    private final String message;

    ErrorCode(HttpStatus status, String message) {
        this.status = status;
        this.message = message;
    }

    public HttpStatus getStatus() {
        return status;
    }

    public String getMessage() {
        return message;
    }
}
